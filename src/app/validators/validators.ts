...
export const passwordMatchValidator: ValidatorFn = (control: AbstractControl): ValidationErrors | null => {
  ...
  let confirmPasswordErrors: ValidationErrors | null = {...confirmPassword.errors};
  const passwordDoNotMatch = password.value !== confirmPassword.value;

  if (passwordDoNotMatch) {
    confirmPasswordErrors = { ...confirmPasswordErrors, passwordMismatch: true };
  } else {
    delete confirmPasswordErrors?.['passwordMismatch'];
  }

  confirmPasswordErrors = Object.keys(confirmPasswordErrors).length === 0
    ? null
